...
class Task12_28:
    ...
    def solution(self):
        ...
        n = len(input_list)
        sum_of_comparing = Sort_algorithms().shaker_sort(input_list)

        File_manager().file_write(file_name=self.output_file, text="list: " + str(input_list) + "\n", mode='a')
        File_manager().file_write(file_name=self.output_file, text="comparing for shacker sort: " + str(sum_of_comparing) + "\n", mode='a')
